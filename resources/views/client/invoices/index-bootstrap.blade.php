  ...
  <div class="d-flex flex-column gap-3">
    @forelse ($invoices as $invoice)
      <div class="card-public p-4 d-flex align-items-center justify-content-between gap-3 flex-wrap">
        <div class="min-w-0">
          <a href="{{ route('client.invoices.show.bootstrap-preview', $invoice) }}" class="fw-semibold text-dark text-decoration-none" style="font-size:15px">
        ...
        <div class="d-flex align-items-center gap-3">
          ...
          @if (in_array($invoice->status, ['unpaid', 'overdue']))
            <a href="{{ route('client.invoices.show.bootstrap-preview', $invoice) }}" class="btn btn-theme">
              <i class="fa-solid fa-credit-card" style="font-size:11px"></i> Bayar
            </a>
          @else
            <a href="{{ route('client.invoices.show.bootstrap-preview', $invoice) }}" class="btn btn-outline-secondary">Detail</a>
          @endif
        </div>
      </div>
    @empty
